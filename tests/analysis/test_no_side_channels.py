"""Static AST audit of ``loki.analysis`` for forbidden side channels (task 21).

Walks every ``.py`` under :mod:`loki.analysis` and fails on:
...
- direct ``time.time()`` / ``time.monotonic()`` / ``time.perf_counter()`` /
  ...
  (:mod:`loki.analysis.timing`)
- ``datetime.now(...)`` calls outside the pipeline module
  (:mod:`loki.analysis.pipeline` — the only place that records the
  run-start timestamp).
- ``os.environ`` attribute access anywhere in the package

Implements Property 51 (R15.3 + R15.4): no environmental side
channels affect ``ImageAnalysisReport`` contents.

Mirrors :mod:`tests.classification.test_no_side_channels`,
:mod:`tests.extraction.test_no_side_channels`, and
...
#: Module where ``time.*`` clock access is allowed. The analysis
#: subsystem follows the extraction / classification pattern of
#: pinning all clock access to a single ``timing.py`` module that
#: exposes a ``Stopwatch`` context manager.
_ALLOWED_TIME_MODULE: str = "loki.analysis.timing"

#: Module where ``datetime.now()`` access is allowed. Per the design,
#: the pipeline orchestrator is the only place that captures the
#: run-start ``datetime.now(UTC)`` (R1.6); the matching, pairing,
#: scoring, posture, finding-emitter, and report modules must not.
_ALLOWED_DATETIME_MODULE: str = "loki.analysis.pipeline"
...
def _module_name_for(path: Path) -> str:
    """Map a filesystem path under loki/analysis/ to a dotted module name."""
    package_root = Path(loki.analysis.__path__[0]).parent.parent  # repo root
...
def test_source_files_were_found(source_files: list[Path]) -> None:
    ...
    # The analysis subsystem ships twelve modules at Wave 6 close.
    assert len(source_files) >= 12


def test_no_forbidden_module_imports(source_files: list[Path]) -> None:
    """No module under ``loki.analysis`` imports a banned dependency."""
    ...
    assert violations == [], f"forbidden module imports found in loki.analysis: {violations}"
                ...
                if node.module == "os":
                    for alias in node.names:
                        if alias.name == "environ":
                            violations.append((str(path), node.lineno))
    assert violations == [], f"os.environ access found in loki.analysis: {violations}"
...
def test_no_datetime_now_outside_pipeline_module(source_files: list[Path]) -> None:
    ...
    The pipeline orchestrator captures the run-start timestamp (R1.6);
    no other module in the subsystem may introduce wall-clock
    dependence.
    """
    violations: list[tuple[str, int, str]] = []
    for path in source_files:
