...
- Cancellation primitive: a `bool` flag (CPython-GIL-protected attribute write); D3 marks `threading.Event` migration as a forward-tracked nit, not a v1 blocker.
...
### `bool` flag → `threading.Event` migration (R24.2, D3)

`ExtractionWorker._cancelled` is currently a `bool`. The CPython GIL makes the attribute write atomic in practice; v1 ratifies this. Migration to `threading.Event` (matching `BaselineLoadWorker` and `AnalysisWorker`) is forward-tracked as a non-blocking nit and is OUT OF SCOPE for v1.
